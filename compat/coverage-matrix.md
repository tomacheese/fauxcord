...
- discord.py, Nextcord, Pycord, hikari, Serenity: verifier code is scaffolded and ready (`Dockerfile` + verify script present under `compat/<dir>/`), but the run has not yet been executed in this environment; rows remain `-` pending a Docker run (see `compat/README.md` "Known run limitations").
- interactions.py, DSharpPlus5, Discord4J, Javacord, Kord, Twilight, Concord, Sleepy: not yet scaffolded; tracked as follow-up per the plan's task breakdown (`docs/superpowers/plans/2026-07-04-issue-68-library-compat.md`).
- discordgo: excluded from this snapshot even though a verifier and a `results/discordgo.json` exist. That result file is stale — every row is `401 Unauthorized`, a systemic auth/base-URL wiring failure from before the endpoint-variable-override fixes (`EndpointGuilds`, `EndpointChannels`, etc. — see `go-discordgo/verify.go`) that are currently in progress on that track. Rows are left `-` pending a fresh run; do not transcribe the stale file's per-endpoint verdicts.
