"""py-cord (Pycord) 2.6.1 compatibility verifier.

Pycord is a discord.py fork (split off around the discord.py 1.7/2.0 era,
like nextcord) that added extensive application-command / UI-component
support on top of a REST object-model API that stayed close to upstream
discord.py for the plain-REST surface this harness exercises
(Client/Guild/TextChannel/Message/Webhook/Thread; no interaction/
application-command endpoints are in common/endpoints.json). Each endpoint
below is exercised through Pycord's *public* high-level API, never
`discord.http.HTTPClient` directly, mirroring the methodology used in
compat/python-discordpy/verify.py and compat/python-nextcord/verify.py.

IMPORTANT (interop hazard, see compat/python-pycord/requirements.txt): Pycord's
PyPI distribution name is `py-cord`, but its import name is `discord` -- the
same import name used by discord.py itself (and by nextcord's own optional
compat shims, though nextcord's primary import name is `nextcord`). This
directory's requirements.txt must therefore install `py-cord` and nothing
else from the discord.py-fork family, or `import discord` would resolve
unpredictably.

Key decisions worth documenting explicitly (same style as the discord.py and
nextcord verifiers for the same class of decisions). Confidence levels are
...
* Login (confidence: high): per the same reasoning, `discord.Client.login()`
  ...
  subclass adding application-command dispatch) as its headline entry
  point, but plain `discord.Client` remains available and sufficient for
  the pure-REST surface tested here, so it is used to keep this verifier
  minimal and avoid pulling in command-framework machinery unrelated to the
  86 canonical endpoints.
* Pins (confidence: medium -- flagged for review): discord.py 2.7.1 (see
  ...
  API. Whether Pycord 2.6.1 made the same migration was not directly
  confirmed against Pycord's source in this session. Pycord is a more
  actively maintained fork than nextcord and plausibly tracked this
  Discord-mandated migration, so -- by the same analogy reasoning applied
  in compat/python-nextcord/verify.py -- we assume it did, exercising the
  new-format endpoints for real and recording the legacy
  `/channels/{id}/pins*` endpoints as `n-a`. If Pycord 2.6.1 in fact still
  targets the legacy API, this assumption is wrong and the pass/n-a
...
* Reaction removal for an explicit (non-self) `{user_id}`: assumed to share
  ...
  unmodified from the pre-fork codebase. Same `n-a` treatment as the other
  two verifiers in this harness, same reasoning: Fauxcord's setup only
  registers one user, so the explicit-{user_id} branch cannot be driven
  through the public API without a fabricated id.
* OAuth2: Pycord's bot `Client`/`Bot` is assumed not to implement the OAuth2
  *authorization code* flow (a discord.py-inherited limitation not directly
  relevant to Pycord's own focus on slash commands/UI), so
...
* Thread search (`GET /channels/{channel_id}/threads/search`): no public
  wrapper confirmed; genuinely uncertain whether Pycord 2.6.1 added one
...
* DELETE-last ordering: identical fix to compat/python-discordpy/verify.py,
  compat/python-nextcord/verify.py, compat/js-oceanic/verify.mjs, and
...
# expects); Pycord's Client.login() is assumed to want the raw token
    ...
    try:
        ...
        calls: dict[str, CallEntry] = {
            ...
            "DELETE /channels/{channel_id}/pins/{message_id}": (
                None,
                "assumed (medium confidence, see module docstring) that Pycord 2.6.1's "
                ...
                "confirmed against Pycord's source in this session, so the legacy API "
            ...
            "GET /channels/{channel_id}/threads/search": (
                None,
                "uncertain whether Pycord 2.6.1 exposes a public wrapper for the thread "
            ...
            "DELETE /guilds/{guild_id}": (
                None,
                "no public wrapper: Pycord's Guild model is assumed, like discord.py, to "
            ...
            "GET /oauth2/@me": (
                None,
                "Pycord's bot Client/Bot is assumed, like discord.py, to not implement the "
        ...
        # compat/python-discordpy/verify.py, compat/python-nextcord/verify.py,
        ...
        for entry in ordered:
            ...
            if call_entry is None or call_entry[0] is None:
                note = (
                    ...
                    else "no high-level Pycord method found for this endpoint"
                )
        ...
        print(f"pycord done: {pass_count}/{len(ordered_results)} pass")
    finally:
