...
describe('versionMiddleware', () => {
  ...
  it('handles v10 paths correctly', async () => {
    const resource = await app.request('/api/v10/channels/123')
    expect(resource.status).toBe(200)
  })

  it('handles paths without an explicit version', async () => {
    const resource = await app.request('/api/channels/123')
    expect(resource.status).toBe(200)
  })

  it('returns 400 for v9 paths', async () => {
    const resource = await app.request('/api/v9/channels/123')
  ...
  it('returns 400 for v6 paths', async () => {
    const resource = await app.request('/api/v6/channels/123')
    expect(resource.status).toBe(400)
  })
})
