  ...
  const applyUpdate = database.transaction(() => {
    if (payload.username !== undefined) {
      database
        ...
        .run(payload.username, bot.user_id)
      database
        .prepare('UPDATE bots SET username = ? WHERE token = ?')
        .run(payload.username, botToken)
    }

    if (payload.avatar !== undefined) {
      database
        .prepare('UPDATE users SET avatar = ? WHERE id = ?')
        .run(payload.avatar, bot.user_id)
      database
        .prepare('UPDATE bots SET avatar = ? WHERE token = ?')
        .run(payload.avatar, botToken)
    }
  })
  applyUpdate()

  return getBotUser(database, botToken)
}
 ...
 * @returns User object, or null
 */
export function getUser(database: Database, userId: string): UserObject | null {
  const user = database
    .prepare('SELECT * FROM users WHERE id = ?')
    .get(userId) as
    | {
...
export function getApp(
  ...
  const user = getUser(database, bot.user_id)
  if (!user) return null
