...
export function resetTestData(db: Database, token?: string): void {
  if (token) {
    ...
    db.prepare(
      "DELETE FROM messages WHERE author_token = ?",
    ).run(token);
    db.prepare(
...
export function getTestMessages(
  db: Database,
  channelId: string,
): { id: string; content: string; author_token: string | null; created_at: string }[] {
  return db
    .prepare("SELECT id, content, author_token, created_at FROM messages WHERE channel_id = ? ORDER BY id")
    .all(channelId) as { id: string; content: string; author_token: string | null; created_at: string }[];
}
