...
import { discordError, validationError } from '../errors'
...
const invalid = (field = 'body') =>
  validationError({
    [field]: {
      _errors: [
        { code: 'BASE_TYPE_BAD_FORMAT', message: 'Invalid form body.' },
      ],
    },
  }).body
...
export function createPartnerSdkRoutes(db: Database): Hono<AppEnv> {
  ...
  app.post('/partner-sdk/token/bot', async (c) => {
    ...
    return c.json(
      issueProvisionalToken(
        db,
        client?.client_id ?? c.get('bot')?.user_id ?? '0',
  ...
  app.put(
    ...
    async (c) => {
      ...
      if ([userId1, userId2, messageId].some((id) => !SNOWFLAKE.test(id)))
        return c.json(invalid(), 400)
      ...
      db.prepare(
        'INSERT INTO partner_sdk_moderation (user_id_1, user_id_2, message_id, metadata) VALUES (?, ?, ?, ?) ON CONFLICT(message_id) DO UPDATE SET metadata = excluded.metadata'
      ).run(userId1, userId2, messageId, JSON.stringify(metadata))
      return c.body(null, 204)
    }
  )
  return app
