...
describe('Channel Permissions API', () => {
  ...
  it('PUT creates an overwrite and returns 204, reflected in the channel', async () => {
    ...
    expect(getChannel(database, channelId)?.permission_overwrites).toEqual([
  ...
  it('PUT upserts without duplicating', async () => {
    ...
    await put('1024')
    await put('8')
    const overwrites = getChannel(database, channelId)?.permission_overwrites
  ...
  it('PUT returns 400 (50035) when type is missing', async () => {
    ...
    expect(((await resource.json()) as { code: number }).code).toBe(50_035)
  })
  ...
  it('PUT returns 404 (10003) for an unknown channel', async () => {
    ...
    expect(((await resource.json()) as { code: number }).code).toBe(10_003)
  })
