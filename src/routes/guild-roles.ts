 ...
 */
export function createGuildRoleRoutes(database: Database): Hono {
  const app = new Hono()
  ...
  app.get('/guilds/:guildId/roles', (c) => {
    ...
    const roles = getGuildRoles(database, guildId)
    return c.json(roles)
  })
  ...
  app.post('/guilds/:guildId/roles', async (c) => {
    ...
    const role = createRole(database, {
      roleId,
      guildId,
      ...payload,
    })
  ...
  app.patch('/guilds/:guildId/roles/:roleId', async (c) => {
    ...
    const updated = updateRole(database, guildId, roleId, payload)
  ...
  app.delete('/guilds/:guildId/roles/:roleId', (c) => {
    ...
    const isDeleted = didDeleteRole(database, guildId, roleId)
    if (!isDeleted) {
      const error = discordError(
        DiscordErrorCode.UNKNOWN_ROLE,
        'Unknown Role',
