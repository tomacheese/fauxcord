...
describe('Channels API', () => {
  ...
  beforeEach(() => {
    ...
    app = new Hono()
    app.route('/', createChannelRoutes(database, BASE_URL))
  ...
  describe('GET /channels/:channelId', () => {
    it('retrieves a channel', async () => {
      const resource = await app.request(`/channels/${channelId}`, {
    ...
    it('returns 404 for a non-existent channel', async () => {
      const resource = await app.request('/channels/999999999999999999', {
        headers: { Authorization: token },
      })
    ...
    it('returns the thread shape (with thread_metadata) for a thread channel', async () => {
      ...
      const resource = await app.request(`/channels/${threadId}`, {
        headers: { Authorization: token },
      })
      expect(resource.status).toBe(200)
      const body = (await resource.json()) as Record<string, unknown>
