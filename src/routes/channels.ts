 ...
 */
export function createChannelRoutes(db: Database, baseUrl: string): Hono<AppEnv> {
  ...
  app.get("/channels/:channelId/messages/pins", (c) => {
    ...
    return c.json({
      items: pins.map((msg) => ({
        pinned_at: new Date(pinnedAtMap.get(msg.id) ?? msg.timestamp).toISOString(),
        message: msg,
      })),
      has_more: false,
    });
  ...
  app.post("/channels/:channelId/messages", async (c) => {
    ...
    if (!bot) {
      ...
      if (authHeader) {
        bot = db.prepare("SELECT * FROM bots WHERE token = ?").get(authHeader) as BotRecord | undefined;
      }
    }
    ...
    let payload: ReturnType<typeof Object.create>;
    let attachmentFiles: { name: string; data: ArrayBuffer; type: string }[] = [];
  ...
  app.patch("/channels/:channelId/messages/:messageId", async (c) => {
    ...
    const payload = await c.req.json<
      Pick<MessageCreatePayload, "content" | "embeds">
    >();
  ...
  app.post("/channels/:channelId/messages/bulk-delete", async (c) => {
    ...
    const messagesMatch = rawBody.match(/"messages"\s*:\s*\[([^\]]*)\]/);
