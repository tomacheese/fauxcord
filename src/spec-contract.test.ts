...
function getResponseSchemaName(entry: SpecEndpoint): string | null {
  ...
  >
  const pathObject = specPaths[entry.specPath]
  if (!pathObject) return null
  const operation = pathObject[entry.method] as OperationType | undefined
...
function isArrayResponse(entry: SpecEndpoint): boolean {
  ...
  >
  const pathObject2 = specPaths2[entry.specPath]
  if (!pathObject2) return false
  const operation = pathObject2[entry.method] as ArrayCheckOperation | undefined
...
describe('Discord spec contract tests', () => {
  // Declared inside this describe block (not at module top level) so
  // beforeAll's assignment isn't a top-level reassignment from a nested
  // function; mirrors integration.test.ts's fixture pattern.
  ...
  beforeAll(() => {
    ;({ db: database, app } = createFullTestApp())
    ...
    seedBot(database, token, BOT_USER_ID)
    const guildId = seedGuild(database, token, '666666666666666666')
    ...
    const channelId = seedChannel(database, guildId, '777777777777777777')
    ...
    const messageId = seedMessage(database, channelId, BOT_USER_ID, token)

    // Seed a separate webhook-authored message for GET/PATCH on webhook message endpoints.
    const webhookMessageId = seedMessage(
      database,
      channelId,
      webhookId,
      'webhook'
    ...
    const roleId = seedRole(database, guildId)
    ...
    const memberId = seedMember(database, guildId)
    const emojiId = seedEmoji(database, guildId, BOT_USER_ID)
    const inviteCode = seedInvite(database, channelId, guildId, BOT_USER_ID)
    ...
    database
      .prepare('INSERT INTO thread_members (thread_id, user_id) VALUES (?, ?)')
  ...
  for (const entry of contractEntries) {
    ...
    it(label, async () => {
      ...
      const resource = await app.request(path, { ...init, headers })

      expect(
        resource.status,
        `Expected ${entry.successStatus} but got ${resource.status} for ${label}`
      ).toBe(entry.successStatus)

      // 204 responses have no body — nothing to validate
      if (entry.successStatus === 204) return

      const body: unknown = await resource.json()
      ...
      if (isArray) {
        ...
        for (const [index, item] of (body as unknown[]).entries()) {
          const isValid = validate(item)
          if (!isValid) {
            throw new Error(
              `Schema validation failed for ${label} item[${index}]:\n` +
